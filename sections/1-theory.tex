...
\mrnote{Nowhere in the HE definitions below do we actually use circuit families?}
...
\begin{definition}[Uniform circuit family \cite{Arora}]
...
\mrnote{Behöver jag referera i definitionerna?}
...
In this paper, we consider homomorphic encryption schemes. These schemes include a fourth algorithm, Eval, called the evaluation algorithm which is used by the server calculating on encrypted data.
...
\mbnote{
circular security - present the alternative (a long chain of keys where the next public key encrypts the previous public key. See TR18
}
...
\mrnote{Does this analogy even make sense? There is no double encryption in my Bootstrapping algorithm?}
A helpful analogy is to consider a powerful river blocked by an old dam. The dam will not last much longer so it will be replaced. An intuitive approach is to first build a new dam placed after the old dam before demolishing it. In the case of bootstrapping, the old dam is the noisy ciphertext, the new dam is a fresh encryption and the demolition represents homomorphic decryption.
...
\mbnote{Pseudocode for bootstrapping algorithm here?}

\mbnote{Correctness here?}
