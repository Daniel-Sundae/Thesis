...
The underlying assumption is that the HE scheme can evaluate $\hat{C}_c$. We have shown that it is possible to use a non-homomorphic encryption scheme to encrypt the message and still perform homomorphic computation on it. Key-switching transforms a ciphertext encrypted under a non-HE scheme to an evaluated ciphertext encrypted under a HE scheme.

Key-switching is a useful optimization technique for increasing the efficiency of the system. Unfortunately however, it does not allow for further computation on the evaluated ciphertext, which means that the function $C$ is constrained such that $\hat{C}_c$ is in the set of permissible circuits. To see why further computation is impossible, denote the evaluated ciphertext $c_c \stackrel{\mathrm{def}}{=} \operatorname{Eval}(hpk,\hat{C}_c, sk')$ and consider computable function $f$ and let $\hat{f}_{c_c}(\cdot) = f(\operatorname{Dec}(\cdot,c_c))$. Then, $\operatorname{Dec}(hsk,\operatorname{Eval}(hpk,\hat{f}_{c_c}, sk')) = f(\operatorname{Dec}(sk, c_c))$. However, since $c_c$ is encrypted under the HE scheme, the non-homomorphic decryption algorithm applied to $c_c$ does not make sense. 

To further compute on the evaluated ciphertext, the decryption algorithm and the secret key must originate from the HE scheme. We therefore only consider the HE scheme. The downside is that encryption and homomorphic decryption is made using the HE scheme's algorithms instead of the faster non-HE schemes, but the upside is that we can perform further computation on the evaluated ciphertext. Let $(\text{KeyGen, Enc, Dec, Eval})$ be a HE scheme, $(pk, sk) \leftarrow \operatorname{KeyGen}(\lambda)$ and $sk' \leftarrow \operatorname{Enc}(pk, sk)$. Since the secret key is encrypted using its corresponding public key, circular security is assumed. Let $\hat{C}_c$ be defined as before. Again, $c_c \stackrel{\mathrm{def}}{=} \operatorname{Eval}(pk,\hat{C}_c, sk')$ is a valid evaluated ciphertext of $C(m)$. The main difference is that we can embed $c_c$ into another function $C_1$ to further compute homomorphically on $c$. 
% \mbnote{
% IDEA FOR OUTLINE:
